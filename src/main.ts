...
import type {Pica, PicaOptions, PicaResizeOptions} from 'pica'
import {Compressed} from './compressed.js'
import type {IDimension} from './interfaces/dimension.js'
import {fixRotation} from './rotate.js'
...
export interface ICompressorOptions {
  ...
  maxHeight?: number
  maxPixels?: number
  picaConfig?: PicaOptions
  ResizeOptions?: PicaResizeOptions
}

const DEFAULT_RESIZE_OPTIONS: PicaResizeOptions = {
  quality         : 3,
  unsharpAmount   : 80,
  unsharpRadius   : 0.6,
  unsharpThreshold: 2,
...
export class Compressor implements ICompressorOptions {
  ...
  constructor(options: ICompressorOptions) {
    ...
    if (options.maxPixels != null) {
      this.maxPixels = options.maxPixels
    }

    if (options.picaConfig != null) {
      this.picaConfig = options.picaConfig
    }
    ...
    this.pica = pica(options.picaConfig)
  }
  ...
  compress(dataUrl: string): Compressed {
    const deferred = pDefer<HTMLCanvasElement>()
    ...
    img.onload = () => {
      ...
      void fixRotation(img, dimen, canvas, ctx)
        .then(async() => {
